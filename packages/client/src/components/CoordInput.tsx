 ...
 *   - Arrow keys / W/S move the cursor in screen-relative directions
 *   - Input box shows current cursor position
 *
 * When user types:
 *   - Manual coordinate entry (x,y,z or xyz)
 *   - Enter or click "落子" to place
 *
 * Key mapping (screen-relative, based on camera at [18,-18,16]):
 *   ← → : X axis (left/right)
 *   ↑ ↓ : Z axis (up/down)
 *   W/S : Y axis (forward/backward, away from/toward camera)
 */
...
export function CoordInput({ onPreview }: CoordInputProps) {
  ...
  // Whether user is manually typing (true) or using cursor navigation (false)
  const [manualMode, setManualMode] = useState(false);
  // Current cursor position in user coords
  ...
  // Compute AI best move for initial cursor position
  useEffect(() => {
  ...
  const handleSubmit = useCallback(() => {
    ...
    if (snapshot.winner !== Stone.EMPTY) {
      setError("游戏已结束"); return;
    }
    if (snapshot.board[idx] !== Stone.EMPTY) {
      setError("该位置已有棋子"); return;
    }
